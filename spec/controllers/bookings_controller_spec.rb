...
RSpec.describe BookingsController, type: :controller do
  describe 'GET available_slots' do
    context 'When params are valid' do
      ...
      it 'returns 200 ok' do
        get :available_slots, params: { date: '01/01/2030', duration: 60 }

        expect(response.status).to eq(200)
      end

      it 'returns slots in the json response' do
        get :available_slots, params: { date: '01/01/2030', duration: 60 }

        expect(JSON.parse(response.body)['success']).to be_truthy
        expect(JSON.parse(response.body)['available_slots']).to eq(expected_slots)
      end
    end

    context 'When params are not valid' do
      it 'returns 422' do
        get :available_slots, params: { date: '01/01/2010', duration: 60 }
  ...
  describe 'POST create' do
    ...
    context 'When params are not valid' do
      it 'returns 422' do
        post :create, params: { booking_date: '01/01/2010', duration_in_minutes: 60, slot: '15:00' }
